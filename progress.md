...
- [x] **Một lần đăng = MỘT commit** (Git Data API: blob → tree → commit → ref). Trước đây mỗi lần đăng tạo **3 commit** (ảnh → `.md` → `index.json`) ⇒ 3 lượt deploy, làm **cạn hạn mức deploy của Vercel** (35 commit ngày 23/7, 23 commit ngày 24/7) khiến bản mới không được deploy và ảnh bị 404.
- [x] **Ảnh được “xếp hàng” rồi commit cùng bài**: chèn vào bài là thấy ngay (data URL), khi Xuất bản thì ảnh + bài lên **cùng một commit** ⇒ không bao giờ còn cảnh bài trỏ tới ảnh chưa tồn tại.
- [x] **Dán ảnh (Ctrl+V) ở bất kỳ đâu** trong trang Quản trị + **kéo-thả** ảnh vào ô nội dung hoặc ô ảnh bìa; nhận cả ảnh clipboard **không có tên tệp** và tệp **không có MIME**.
